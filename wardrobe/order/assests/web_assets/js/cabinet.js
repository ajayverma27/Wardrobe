...
$(document).ready(function () {
  ...
  $(document).on('click', '.t4_add_green', function () {
    ...
    $(this).parents('.part2_section_new').html(`<div class="d-flex-center-relative t4_bdr_green flex-column cabinet_first_section1"><img src=${baseurl + 'assests/web_assets/img_web/cross-white.png' } data-idd="${did}" data-cntt="${cnt}" class="cross_t4_remove-img img_invert"><span class="t4-drawer-height " style="display:none">${dw}</span></div>`);
...
$(document).on('click','#gt_tb .new-td-parents',function(){
...
function tab7_btn_valid(){
    ...
    // $(".tab-7-table").find('.img_bord_2').each(function(ind){
    //     if($(this).length > 0)
    //     {
    //         numchecks++;
    //     }
    ...
    //numchecks = parseInt(numchecks) + parseInt(1)
...
//   var doorthickness = 18;
  ...
  tbody += `<tr>
        <th>Výpočet vrchnej časti skrine</th>
        <td>
            <span class="calc_panel">Vrchný materiál skrine : </span>
            <span class="calc_panel">${topmaterial.toFixed(2)}mm2</span>
            <span class="calc_panel">${(topmaterial / 1000000).toFixed(2)}m2</span><br><br>
            <span class="calc_panel">Náklady na vrchný materiál skrine : </span>
  ...
  tbody += `<tr>
        <th>Výpočet dverí skrine</th>
        <td>
            <span class="calc_panel">Materiál dverí skrine : </span>
            <span class="calc_panel">${doormaterial.toFixed(2)}mm2</span>
            <span class="calc_panel">${(doormaterial / 1000000).toFixed(2)}m</span><br><br>

            <span class="calc_panel">Materiál dvierok skrine: </span>
  ...
  tbody += `<tr>
        <th>Výpočet zadnej strany skrine</th>
        <td>
            ...
            <span class="calc_panel">${(cabinetbackmaterial / 1000000).toFixed(2)}m2</span><br><br>
             ...
             <span class="calc_panel">Náklady na materiál zadnej strany skrine :</span>
  ...
  // tbody += `<tr>
  //       <th>Výpočet deliacej steny skrine(1)</th>
  //       <td>
  //           <span class="calc_panel">Materiál deliacej steny skrine(1) : </span>
  ...
  //           <span class="calc_panel">Náklady na materiál deliacej steny skrine(1) : </span>
  ...
  tbody += `<tr>
        <th>Výpočet deliacej steny skrine (X${noofdwall})</th>
        <td>
            <span class="calc_panel">Materiál deliacej steny skrine : </span>
    ...
    //Cabinet shelf cost and material expenditure calculation
     tbody += `<tr>
        <th>Výpočet nákladov na policu a materiálu</th>
        <td>
            
            <span class="calc_panel">Materiál skrine pre všetky police (x${noshelves})</span>
  ...
  //   </tr>`;
            ...
            let sn = $(this).parents('.set_inside_framer').attr('data-subsec');
             ...
             let fronbackmaterialcost = fronbackmaterialm*tdtmaterialcost;
               ...
               gtotal = parseFloat(gtotal)+parseFloat(fronbackmaterialcost);
                ...
                tbody += `<h4>Zásuvka (${(ind+1)}) kalkulácia</h4>
             ...
             //Drawer container front and back ABS
              ...
              gtotal = parseFloat(gtotal)+parseFloat(fronbackabsmaterialcost*2);
                tbody += `
                    ...
                    <span class="calc_panel">${(fronbackabsmaterialcost * 2).toFixed(2)}€</span><br><br>
                `;
    ...
    if($('#table_tab_4final').find('.cab_tab_4 .set_inside_framer .t4-drawer-height').length > 0){
          ...
          gtotal = parseFloat(gtotal)+parseFloat(alldrawersidematerialcost*2);
            tbody += `<h4 class="pb-5">Výpočet zásuvkového kontajnera</h4>
        ...
        let totaldoorhandlecost1 = drawerhandlecost * nodrawer;
        gtotal = parseFloat(gtotal)+parseFloat(totaldoorhandlecost1);
        tbody += `<tr>
            <th>Rukoväte zásuviek</th>
            <td>
                
                <span class="calc_panel">Počet rukovätí zásuviek</span>
                <span class="calc_panel">${nodrawer}</span>
                <br><br>
    ...
    $(this).find('.new-tbl-parents .set_inside_framer').each(function(index){
       ...
       let tt1 = parseFloat(checkn1)+parseFloat(checkp1)
        if(door == 1){
            ...
            tbody +=  `<span class="calc_panel">Počet dverí (${doorinc})</span>
            <br><br>
            ...
            <span class="calc_panel">${totaldoormaterialmcost.toFixed(2)}€</span><hr>`;
             doorinc++;
        }
   ...
   if(doorlength > 0 && door == 1){
    ...
    // let checkp = $(this).parents('.new-tr-parent').prev('tr.set_inside_framer').length;
       ...
       if(f=='f-3'){
        ...
        <br><br>
        <span class="calc_panel">Cena dverí</span>
        <span class="calc_panel">${totaldoormaterialmcost1.toFixed(2)}€</span><hr>`;
...
$(document).on('click','.door-section-confirm',function(){
    ...
    let door = '';
    if(check == 'right'){  //left
         door = `<div class="align-items-start d-flex-center-relative flex-column t4_bdr_black handle-min-height-left">
        ...
        door =  `<div class="align-items-end d-flex-center-relative flex-column t4_bdr_black handle-min-height-right">
            <span class="handle-left t4_bdr_black_1"></span>
